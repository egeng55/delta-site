# Delta Phase 74: maintenance-policy
...
- Routine: docs-only
- Mode: single
- Objective: Add a read-only maintenance policy engine that classifies findings into allowed agent action modes. Use that policy to make maintenance reports and finding-generated phase briefs explicit about approval requirements and forbidden actions.
...
npm run agent:orchestrate -- --phase 74 --name maintenance-policy --routine docs-only
```
...
```bash
npm run agent:phase:start -- --phase 74 --name maintenance-policy --print
```

- Expected branch: `phase-74-maintenance-policy`
- Expected worktree path: `/Users/egeng/delta-worktrees/site-phase-74-maintenance-policy`
...
```bash
npm run agent:phase:handoff -- --phase 74
```
...
- None for the policy foundation. Future phases should tune individual finding policy metadata only after reviewing the current queue and approval requirements.
