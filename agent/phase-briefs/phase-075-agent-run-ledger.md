# Delta Phase 75: agent-run-ledger
...
- Routine: docs-only
- Mode: single
- Objective: Add a report-only agent run ledger that records maintenance inspection results, policy decisions, refused actions, approval requirements, and recommended next commands. Writing a durable run record must require `--write` and must not implement any maintenance finding.
...
npm run agent:orchestrate -- --phase 75 --name agent-run-ledger --routine docs-only
```
...
```bash
npm run agent:phase:start -- --phase 75 --name agent-run-ledger --print
```

- Expected branch: `phase-75-agent-run-ledger`
- Expected worktree path: `/Users/egeng/delta-worktrees/site-phase-75-agent-run-ledger`
...
```bash
npm run agent:phase:handoff -- --phase 75
```
...
- None for the ledger foundation. Future phases should decide when background/report-only routines are required to write run records before acting.
